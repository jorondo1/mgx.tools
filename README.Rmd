...
Bioconductor ones. Be aware that mgx.tools has many dependencies (around 30,
including DADA2 and phyloseq), so the first installation can take a while.

Some functions rely on additional packages that are not installed by default:
tree building (DECIPHER, phangorn), phylogenetic diversity (phangorn, GUniFrac,
picante), variance-stabilizing transformation (DESeq2), and `psflashmelt()`
(data.table). If one is missing when you call such a function, you will be
offered to install it. To install them all upfront:
