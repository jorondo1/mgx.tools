...
tax_glom2 <- function(ps, taxrank) {
  ...
  tax_long <- phyloseq::tax_table(ps) %>%
    data.frame() %>%
  ...
  merged_long <- dplyr::left_join(
    tax_long[,1:taxrank_idx],
    otu_long,
    by = "OTU")

  # Aggregate counts by Sample and taxrank (sum)
  agg_data <- merged_long %>%
    dplyr::group_by(Sample, !!sym(taxrank)) %>%
    dplyr::summarise(Abundance = sum(Abundance, na.rm = TRUE),
                     .groups = "drop") %>%
    dplyr::filter(!!sym(taxrank) != 'Unclassified' & !is.na(!!sym(taxrank)))
  ...
  otu_matrix <- agg_data %>%
    tidyr::pivot_wider(
      ...
      values_fill = 0) %>%
    dplyr::arrange({{ taxrank }}) %>%
    tibble::column_to_rownames(var = taxrank) %>%
    as.matrix()
  ...
  tax_matrix <- tax_long[,1:taxrank_idx] %>%
    dplyr::select(-OTU) %>%
    as.data.frame() %>%
    dplyr::mutate(tax1=!!sym(taxrank)) %>%
    dplyr::distinct(!!sym(taxrank), .keep_all = TRUE) %>%  # Some weird instances of same genera from different families...
    dplyr::arrange(tax1) %>%
    tibble::column_to_rownames('tax1') %>%
    as.matrix()

  # Rebuild the phyloseq object with the aggregated OTU and taxonomy tables
  new_ps <- phyloseq::phyloseq(
    phyloseq::otu_table(otu_matrix, taxa_are_rows = TRUE),
    phyloseq::tax_table(tax_matrix),
    phyloseq::sample_data(ps@sam_data)
  ) %>%
    phyloseq::prune_samples(sample_sums(.) > 0, .) %>%  #remove any empty samples
    phyloseq::prune_taxa(taxa_sums(.) > 0,.) #remove taxa absent from all
